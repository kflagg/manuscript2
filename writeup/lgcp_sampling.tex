...
assess their optimality for mapping intensity using LGCP models via a
simulation study.
...
schemes tended to result in slightly underestimating the mean
(Figure \ref{intlgcp}). For each scheme at each effort level, the median
...
have long right tails with severe overestimates.
...
\begin{figure}
\includegraphics[width=5in]{../graphics/Int-effort-notpaneled-LGCP000004.png}
\caption{Plot of estimation error in the posterior mean vs survey effort for
each plan applied to one realization of a LGCP. Line segments connect the
median at each effort level.}
\label{intlgcp}
\end{figure}
...
estimating the mean (Figure \ref{intclust}). The median errors for the scheme
and effort combinations range between \(-0.13\) and 0.16. IQR decreases
slightly with higher effort. At low effort, all schemes have long-tailed
distributions containing some extreme errors; noteably the serpentine and
SRS schemes have left-tailed distributions while the others are right-tailed.
...
\begin{figure}
\includegraphics[width=5in]{../graphics/Int-effort-notpaneled-Cluster000004.png}
\caption{Plot of estimation error in the posterior mean vs survey effort for
each plan applied to one realization of a LGCP with Clusters process. Line
segments connect the median at each effort level.}
\label{intclust}
\end{figure}
