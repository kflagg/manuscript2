...
inferences, so average prediction variance (APV) should be low. The next
sections compare the schemes in terms of MSPE and APV applied to one realized
LGCP dataset and one realized LGCP with Clusters dataset.
...
distance between the path and arbitrary points in the study site. We define
the distance between a point and the path as the distance between that point
and the nearest point on the path, then take the average over all points in
the study area. This example design has an averge distance from the path of
486 units; the designs used in the simulation study all had average distances
to the path under 130 (with most under 50).
% Move explanation of maximin distance to methods then add to table.
% On second thought, the story might really be about the average.
