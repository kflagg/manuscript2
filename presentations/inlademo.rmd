...
```{r beiinla}
centers <- gridcenters(window(bei), 100, 50)
...
image(unique(bei_df$x), unique(bei_df$y), matrix(bei_df$count, nrow = length(unique(bei_df$x))), asp = 1)

bei_inla <- inla(count ~ 1, family = 'poisson', data = bei_df)
```
