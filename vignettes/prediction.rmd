...
- Finite element approximation $Z(s) = \sum_{i = 1}^{n} z_{i} \psi_{i}(s)$
  where $\mathbf{z} = (z_{1}, \dots, z_{n})')$ is multivariate normal and
  $\{\psi_{i}(s)\}_{i = 1}^{n}$ is a set of piecewise linear basis functions.
...
Sampling effort is accomodated by scaling the integration weights by the
probability that a point would have been observed at that node (given the
...
a (known) detection function [@yuanetal] or a (known) false negative rate for
detection equipment. This adjustment assumes the point process of interest is
observed through a known thinning process and then allows inference back to the
intensity function of the unthinned process.
...
random effects are shifted from method to method, but theses are not separately
indentifyable and the shifts cancel each other out. The methods each have
