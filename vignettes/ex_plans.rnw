...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, fig.path = 'plans_fig/',
...
bim <- function(n, m = pi/3, s = pi/6){
  return(sample(c(-1, 1), n, TRUE) * rnorm(n, m, s))
}

rpm <- function(full_win, max_dist = DIST_MAX, corr = XSECT_LENGTH_CORR,
                seg_min = XSECT_LENGTH_MIN, seg_max = XSECT_LENGTH_MAX,
                angle_dist = bim, a = 1, b = 1, margin = WP_MARGIN){
  full_frame <- Frame(full_win)
  min_x <- min(full_frame$x) + margin
  max_x <- max(full_frame$x) - margin
  min_y <- min(full_frame$y) + margin
  ...
  # The BAR is
  ##  X[i] <- U * (1 - W * X[i - 1])
  # with U ~ Beta(b, a - p) and W ~ Beta(p, a - p).
  # Check if pos_corr, and if so do
  ##  X[i] <- 1 - X[i]

  # To get distance of point to psp, make a psp with one segment starting and
  # ending at the point then use
  ##  min(crossdist(test_point, the_psp, type = 'separation'))
...
The dataset in a \(1500 \times 700\) rectangular region. It will be surveyed
such that only events within 2 units of a path are observed.
...
point
\item Shortest path (traveliing salesperson) through a Latin hypercube
sampling design
